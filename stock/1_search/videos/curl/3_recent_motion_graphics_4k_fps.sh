...
curl --silent --show-error --get "$API_URL" \
  ...
  --data-urlencode "term=motion graphics background" \
  --data-urlencode "page=1" \
  --data-urlencode "per_page=5" \
  --data-urlencode "order=recent" \
  --data-urlencode "filters[category]=motion_graphics" \
  --data-urlencode "filters[resolution][720]=0" \
  --data-urlencode "filters[resolution][1080]=0" \
  --data-urlencode "filters[resolution][2K]=0" \
  --data-urlencode "filters[resolution][4K]=1" \
